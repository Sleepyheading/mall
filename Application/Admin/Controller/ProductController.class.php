...
class ProductController extends CommonController {
    public function index(){
    	// $res=D('Proclass')->relation(true)->select();
    	// p($res);
    	// die;
  //       $conds = array();
  //       $name = $_GET['name'];
  ...
  //       $products=D('Product')->getProducts($conds,$page,$pageSize);
		$class=D('Classify')->getClasses(0);
  //       $count = D("Product")->getNewsCount($conds);
  ...
  //       $this->display();
            ...
            $Page->setConfig('theme','%FIRST% %UP_PAGE% %LINK_PAGE% %DOWN_PAGE% %END% %HEADER%'); 
             $show       = $Page->show();// 分页显示输出
			$list = $Proclass->relation(true)->where('status!='.-1)->order('create_time desc')->limit($Page->firstRow.','.$Page->listRows)->select();
