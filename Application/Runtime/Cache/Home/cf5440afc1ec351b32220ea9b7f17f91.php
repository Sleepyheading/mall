					...
					<tbody>
						<tr class="text-center">
							<td class="text-left" style="vertical-align: middle !important;border-top:none !important">
									...
									<td style="vertical-align: middle !important;border-top:none !important">
										<?php if($v["order_send"] == 0): ?><a class="singg-delete" style="cursor:pointer;text-decoration: none;color:#D9534F">
											等待发货</a>
										
										<?php elseif($v["order_send"] == 1 and $v["order_get"] == 0): ?>
										<span   style="cursor:pointer;text-decoration: none;color:#D9534F" onclick="orderGet(<?php echo ($v["order_id"]); ?>)">
											确认收货</span>
											<input type="hidden"  value="<?php echo ($v["order_id"]); ?>">
										
										<?php else: endif; ?>
										<span  attr-id="<?php echo ($v["cart_id"]); ?>" class="singg-delete" style="cursor:pointer;text-decoration: none;color:#D9534F">
											查看物流</span>
										<?php if($v["order_send"] == 1 and $v["order_get"] == 1): ?><p style="margin-bottom: -40px;padding: 20px;"><a style="cursor:pointer;text-decoration: none;color:#D9534F;" onclick="orderDel(<?php echo ($v["order_id"]); ?>)">删除订单</a></p>
										<?php else: endif; ?>
									</td>
						</tr>
					</tbody>
